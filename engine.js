...
class SabbathGame{
 ...
 reset(){
  this.viewWidth=this.viewWidth||960;this.mode='intro';this.time=0;this.kills=0;this.cam=0;this.events=[];this.powerUses=0;this.combo=0;this.comboAge=0;this.shake=0;this.hitstop=0;this.projectiles=[];this.nextProjectile=0;this.attackTargets=new Set();this.attackBuffer=0;this.jumpBuffer=0;this.stats={hits:0,perfectDodges:0,parries:0,airHits:0,knifeHits:0,damageTaken:0};
  ...
  this.enemies=types.map((type,i)=>({id:i,type,x:xs[i],z:0,hp:TYPES[type].hp,maxHp:TYPES[type].hp,heavy:type==='heavy',phase:[1,4,6,9].includes(i)?'buried':'idle',timer:0,flash:0,face:-1,dead:false,death:0,spawnX:xs[i],attackNo:0,staggerGuard:0,broken:0,walk:false,hitPlayer:false,lockedFace:-1}));this.enemies[9].hp=this.enemies[9].maxHp=480;this.enemies[9].elite=true;this.exit=3750;this.markers=new Set();
 }
 ...
 spawnShot(e,kind,direction){this.projectiles.push({id:this.nextProjectile++,kind,x:e.x+direction*30,z:kind==='wave'?8:82,vx:direction*(kind==='wave'?240:210),life:kind==='wave'?1.15:2.8,damage:kind==='wave'?25:18,dead:false});}
 ...
 tickEnemy(e,dt){
  ...
  if(e.type==='spitter'&&dist<180&&onScreen){e.x-=e.face*cfg.speed*.8*dt;e.walk=true;return}
