...
class SabbathGame{
 ...
 reset(){
  this.mode='intro';this.time=0;this.kills=0;this.cam=0;this.events=[];this.powerUses=0;this.combo=0;this.comboAge=0;this.shake=0;this.hitstop=0;this.projectiles=[];this.nextProjectile=0;this.attackTargets=new Set();this.attackBuffer=0;this.jumpBuffer=0;this.stats={hits:0,perfectDodges:0,parries:0,airHits:0,damageTaken:0};
  this.player={x:130,z:0,vz:0,hp:100,maxHp:100,nav:0,face:1,attack:0,cooldown:0,evade:0,evadeCd:0,inv:0,moving:false,power:0,transform:0,stride:0,counter:0,airAttack:false,lowAttack:false,crouching:false,attackHit:false};
 ...
 attack(){
  ...
  p.airAttack=p.z>16;p.lowAttack=p.crouching&&!p.airAttack;p.attack=p.airAttack?.38:.34;p.cooldown=p.airAttack?.48:([0,.34,.39,.58][this.combo]);p.attackHit=false;p.attackFacing=p.face;this.attackTargets.clear();
 ...
 jump(){const p=this.player;if(this.mode!=='playing'||p.transform>0)return false;if(p.z===0){p.crouching=false;p.vz=450;this.jumpBuffer=0;this.emit('jump');return true}this.jumpBuffer=.12;return false}
 ...
 tick(dt,input={}){
  ...
  if(p.evade>0)p.x+=p.face*590*dt;else if(p.moving)p.x+=axis*(p.crouching?62:p.power>0?190:158)*dt;
