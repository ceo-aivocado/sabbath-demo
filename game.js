...
function press(key,repeat,hold=true){if(game.mode==='won'&&game.chapter>=2&&game.chapter<5&&!repeat&&['Enter',' ','j'].includes(key)){continueJourney();return;}if(key==='Escape'&&!repeat){pause();return}if(game.mode==='house'){if(hold)keys.add(key);if(!repeat&&['j','q','f','Enter'].includes(key))game.interact();return;}if(game.mode!=='playing')return;if(hold)keys.add(key);if(!repeat){if(key==='x')game.player.crouching=game.player.z===0&&game.player.vz<=0&&game.player.evade===0;if(key==='j')game.attack();if(key===' '){crouchToggle=false;game.jump();}if(key==='q'){crouchToggle=false;game.dodge('back');}if(key==='Shift'||key==='k'){const back=inputHeld('x')||crouchToggle;crouchToggle=false;game.dodge(back?'back':'forward');}if(key==='v')game.knife();if(key==='e')game.power()}}
...
function inputHeld(key){return keys.has(key)||[...touchPointers.values()].includes(key)||[...padPointers.values()].some(p=>p.keys.includes(key))}
...
function releasePointer(id,completed=false){const state=padPointers.get(id);if(completed&&state?.backTap&&!state.sliding&&performance.now()-state.backTap.startedAt<=200)lastBackTap={...state.backTap,releasedAt:performance.now()};touchPointers.delete(id);padPointers.delete(id);paintHeld()}
function clearTouch(){lastBackTap=null;crouchToggle=false;touchPointers.clear();padPointers.clear();paintHeld()}
...
movePad.addEventListener('lostpointercapture',e=>{if(padPointers.has(e.pointerId))crouchToggle=false;releasePointer(e.pointerId)});
canvas.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse'&&e.button===0){press('j',false);canvas.focus()}});
window.addEventListener('pointerup',e=>{if(e.pointerType==='mouse'&&!touchPointers.has(e.pointerId)&&!padPointers.has(e.pointerId))keys.delete('j');releasePointer(e.pointerId,true)});
window.addEventListener('pointercancel',e=>{if(padPointers.has(e.pointerId)){crouchToggle=false;lastBackTap=null;}releasePointer(e.pointerId)});
