...
const assets={demonWalk:'assets/yaromir-demon-walk.png',walk:'assets/yaromir-walk.png',hero:'assets/yaromir-atlas.png',demon:'assets/yaromir-demon-atlas.png',special:'assets/enemies-special.png',enemy:'assets/enemies-atlas.png',bg:'assets/background.png',props:'assets/props-atlas.png'};
...
function keyName(e){return ({KeyA:'a',KeyD:'d',KeyZ:'j',KeyJ:'j',KeyK:'k',KeyC:'e',KeyE:'e',KeyW:' ',Space:' ',ArrowLeft:'a',ArrowRight:'d',ArrowUp:' ',ShiftLeft:'Shift',ShiftRight:'Shift',Escape:'Escape'})[e.code]||e.key}
function press(key,repeat){if(key==='Escape'&&!repeat){pause();return}if(game.mode!=='playing')return;keys.add(key);if(!repeat){if(key==='j')game.attack();if(key===' ')game.jump();if(key==='Shift'||key==='k'){if(keys.has('a')!==keys.has('d'))game.player.face=keys.has('a')?-1:1;game.dodge();}if(key==='e')game.power()}}
window.addEventListener('keydown',e=>{let k=keyName(e);if(['a','d','j','k','e',' ','Shift','Escape'].includes(k)){e.preventDefault();press(k,e.repeat)}});window.addEventListener('keyup',e=>keys.delete(keyName(e)));window.addEventListener('blur',()=>{keys.clear();if(game.mode==='playing')pause()});document.addEventListener('visibilitychange',()=>{if(document.hidden&&game.mode==='playing')pause()});
...
function clearTouch(){touchPointers.clear();for(const b of document.querySelectorAll('[data-key]'))b.classList.remove('held')}
...
for(const b of document.querySelectorAll('[data-key]')){b.addEventListener('contextmenu',e=>e.preventDefault());b.addEventListener('pointerdown',e=>{e.preventDefault();if(game.mode!=='playing')return;b.setPointerCapture(e.pointerId);touchPointers.set(e.pointerId,b.dataset.key);b.classList.add('held');press(b.dataset.key,false)});b.addEventListener('lostpointercapture',e=>releasePointer(e.pointerId))}
function prop(index,x,y,width,alpha=1,flip=false){if(!art.props)return;const boxes=[[0,0,510,550],[520,245,510,305],[1045,55,490,490],[0,742,535,270],[540,565,535,437],[1105,600,430,412]],b=boxes[index],h=width*b[3]/b[2];ctx.save();ctx.globalAlpha=alpha;ctx.translate(Math.round(x),Math.round(y));if(flip)ctx.scale(-1,1);ctx.drawImage(art.props,...b,-width/2,-h,width,h);ctx.restore()}
...
function hero(frame,x,y,alpha=1){
 if(!art.hero)return;const p=game.player,f=heroFrames[frame];if(frame>=1&&frame<=3&&p.moving&&p.z===0&&p.attack===0&&p.evade===0&&walkingHero(x,y,alpha))return;
 ...
 layer(art.hero,1-blend,false);layer(art.demon,blend,true);
}
function enemy(e){
 ...
 const col=windup?2:e.phase==='strike'?3:e.walk?Math.floor(clock*9)%2:0,special=(crouch||spitter)&&art.special,sprite=special?art.special:art.enemy,row=special?(spitter?1:0):(e.heavy?1:0),sw=sprite.width/4,sh=sprite.height/2,scale=e.heavy?.36:crouch?.31:.31;
 ...
 ctx.save();ctx.translate(Math.round(x),Math.round(y));ctx.scale(-e.face,1);ctx.globalAlpha=e.dead?e.death:1;
...
function renderShots(){for(const shot of game.projectiles){const x=shot.x-game.cam;if(x<-100||x>W+100)continue;
 ...
 else{for(let i=4;i>=0;i--)glow(x-i*Math.sign(shot.vx)*9,G-shot.z,11-i,'#ac83d944');rect(x-4,G-shot.z-4,8,8,'#bda0e5');rect(x-1,G-shot.z-2,3,3,'#f1dfff')}
}}
...
function renderBackground(cam,t){rect(0,0,W,H,'#202b3c');if(art.bg){let width=1050,height=700,offset=(cam*.11)%width;for(let i=-1;i<Math.ceil(W/width)+1;i++){ctx.save();ctx.translate(i*width-offset,-105);if(i%2){ctx.translate(width,0);ctx.scale(-1,1)}ctx.drawImage(art.bg,0,0,width,height);ctx.restore()}}
 ...
 for(const p of [[1,820,449,180],[2,1640,450,185],[1,2450,449,195],[1,3030,449,180]]){const x=p[1]-cam;ctx.save();ctx.fillStyle='#05090b55';ctx.beginPath();ctx.ellipse(x,447,p[3]*.44,5,0,0,7);ctx.fill();ctx.restore();prop(p[0],x,p[2],p[3],.85);}
