...
function press(key,repeat){if(key==='Escape'&&!repeat){pause();return}if(game.mode!=='playing')return;keys.add(key);if(!repeat){if(key==='j')game.attack();if(key===' '){crouchToggle=false;game.jump();}if(key==='Shift'||key==='k'){crouchToggle=false;if(keys.has('a')!==keys.has('d'))game.player.face=keys.has('a')?-1:1;game.dodge();}if(key==='e')game.power()}}
...
function render(){fitViewport();ctx.setTransform(1,0,0,1,0,0);ctx.globalAlpha=1;ctx.imageSmoothingEnabled=false;ctx.save();const intro=game.mode==='intro',cam=intro?Math.sin(clock*.035)*60+100:game.cam;let shake=game.mode==='playing'?game.shake:0;ctx.translate((rand(clock)*2-1)*shake,(rand(clock+2)*2-1)*shake*.4);renderBackground(cam,clock);
 ...
 ctx.restore();const vignette=ctx.createRadialGradient(W/2,H*.5,H*.18,W/2,H*.5,W*.65);vignette.addColorStop(0,'#050c1300');vignette.addColorStop(1,game.player.inv>.6?'#711e32a8':'#04090fd1');ctx.fillStyle=vignette;ctx.fillRect(0,0,W,H);rect(0,0,W,9,'#070d13');rect(0,H-9,W,9,'#070d13');
}
