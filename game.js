...
function initAudio(){if(audio){audio.resume();return}audio=new(window.AudioContext||window.webkitAudioContext)();master=audio.createGain();master.gain.value=.32;master.connect(audio.destination);ambient=audio.createGain();ambient.gain.value=game.scene==='house'?.2:.8;ambient.connect(master);for(const f of [55,82.41,110.2]){let o=audio.createOscillator(),g=audio.createGain();o.frequency.value=f;o.type='sine';g.gain.value=.022;o.connect(g).connect(ambient);o.start()}let size=audio.sampleRate*4,b=audio.createBuffer(1,size,audio.sampleRate),d=b.getChannelData(0);for(let i=0;i<size;i++)d[i]=Math.random()*2-1;let s=audio.createBufferSource(),f=audio.createBiquadFilter(),g=audio.createGain();s.buffer=b;s.loop=true;f.type='lowpass';f.frequency.value=240;g.gain.value=.08;s.connect(f).connect(g).connect(ambient);s.start()}
...
function start(fresh=false){if(!ready||phonePortrait())return;clearTouch();if(fresh){savedCheckpoint=null;try{localStorage.removeItem(CHECKPOINT_KEY)}catch{}}
 ...
 particles.length=0;ghosts.length=0;keys.clear();$('intro').hidden=true;$('menu').hidden=true;$('story-copy').hidden=true;$('hud').hidden=false;storyUntil=0;transition=.45;canvas.focus();if(!audio&&!soundOn)soundToggle();if(ambient)ambient.gain.setTargetAtTime(game.scene==='house'?.2:.8,audio.currentTime,.3);}
...
function pause(){clearTouch();if(['playing','house'].includes(game.mode)){pausedMode=game.mode;game.mode='paused';showMenu('ПАУЗА','Тишина между ударами','Путь дождётся тебя.',true)}else if(game.mode==='paused'){game.mode=pausedMode;$('menu').hidden=true;canvas.focus()}keys.clear()}
...
for(const b of document.querySelectorAll('[data-key]')){b.addEventListener('contextmenu',e=>e.preventDefault());b.addEventListener('pointerdown',e=>{e.preventDefault();if(!['playing','house'].includes(game.mode))return;if(b.dataset.key==='x'){crouchToggle=!crouchToggle;game.player.crouching=crouchToggle&&game.player.z===0;return;}b.setPointerCapture(e.pointerId);touchPointers.set(e.pointerId,b.dataset.key);b.classList.add('held');press(b.dataset.key,false)});b.addEventListener('lostpointercapture',e=>releasePointer(e.pointerId))}
...
function hero(frame,x,y,alpha=1){
 if(!art.hero)return;const p=game.player;if(p.throwTime>0&&!p.crouching&&throwingHero(x,y,alpha))return;if(p.crouching&&crouchingHero(x,y,alpha))return;const f=heroFrames[frame];if(frame>=1&&frame<=3&&p.moving&&p.z===0&&p.attack===0&&p.evade===0&&walkingHero(x,y,alpha))return;
...
function guard(e){
 const img=art.guards,x=e.x-game.cam,captain=e.type==='captain';if(!img||x<-190||x>W+190||e.dead&&e.death<=0)return;
 const windup=e.phase==='windup',strike=e.phase==='strike',col=strike?(e.move==='thrust'?2:3):windup?1:e.walk?Math.floor(clock*6+e.id)%2:0;
 const frames=[[[0,0,435,443,280,392],[480,0,380,443,247,392],[864,0,471,443,341,392],[1336,0,438,443,287,392]],[[0,443,439,444,225,379],[469,443,415,444,212,379],[885,443,470,444,349,379],[1356,443,418,444,273,381]]],f=frames[captain?1:0][col],scale=captain?.38:.43;
 ...
 if(windup){const low=['sweep','wave'].includes(e.move),slam=e.move==='slam',z=low?15:slam?3:83;ctx.save();ctx.strokeStyle=low?'#eecb83a0':'#e6a592a0';ctx.lineWidth=2;ctx.setLineDash(slam?[]:[5,7]);ctx.beginPath();ctx.moveTo(e.move==='wave'?x-210:x,G-z);ctx.lineTo(e.move==='wave'?x+210:x+e.lockedFace*(slam?125:185),G-z);ctx.stroke();ctx.font='bold 17px Arial';ctx.fillStyle='#efd6a4';ctx.textAlign='center';ctx.fillText(low?'↑':slam?'↔':'↓',x,G-(captain?168:145));if(e.move==='wave'&&e.enraged){ctx.font='10px Arial';ctx.fillText('2 ×',x,G-182);}ctx.restore();}
...
function gunner(e){
 const img=art.gunner,x=e.x-game.cam;if(!img||x<-160||x>W+160||e.dead&&e.death<=0)return;
 let row=0,col=e.walk?Math.floor(clock*8)%2:0;if(e.phase==='windup'){row=e.shotLow?1:0;col=e.shotLow?1:2;}else if(e.phase==='strike'){row=e.shotLow?1:0;col=e.shotLow?2:3;}else if(e.phase==='recover'){row=1;col=0;}else if(e.phase==='hurt'||e.dead){row=1;col=3;}
 const frames=[[[0,0,410,443,250,418],[436,0,403,443,251,418],[872,0,455,443,293,418],[1335,0,439,443,325,418]],[[0,443,410,444,231,404],[420,443,450,444,310,404],[875,443,465,444,295,404],[1410,443,364,444,240,404]]],f=frames[row][col];
 ctx.save();ctx.fillStyle='#03080c66';ctx.beginPath();ctx.ellipse(x,G+3,27,6,0,0,7);ctx.fill();ctx.translate(Math.round(x),G);ctx.scale(-e.face,1);ctx.globalAlpha=e.dead?e.death*e.death:1;if(e.dead)ctx.rotate((1-e.death)*.75);if(e.flash>0)ctx.filter='brightness(1.7)';ctx.drawImage(img,f[0],f[1],f[2],f[3],-f[4]*.30,-f[5]*.30,f[2]*.30,f[3]*.30);ctx.restore();
 if(e.phase==='windup'){const z=e.shotLow?48:88;ctx.save();ctx.strokeStyle=e.shotLow?'#d9ae69a0':'#e6928699';ctx.setLineDash([4,8]);ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(x+e.lockedFace*85,G-z);ctx.lineTo(x+e.lockedFace*430,G-z);ctx.stroke();ctx.fillStyle=e.shotLow?'#f3ce84':'#eab9a4';ctx.font='bold 15px Arial';ctx.textAlign='center';ctx.fillText(e.shotLow?'↑':'↓',x,G-145);ctx.restore();}
...
function render(){ctx.setTransform(1,0,0,1,0,0);ctx.globalAlpha=1;ctx.imageSmoothingEnabled=false;ctx.save();const intro=game.mode==='intro',cam=intro?Math.sin(clock*.035)*60+100:game.cam;let shake=game.mode==='playing'?game.shake:0;ctx.translate((rand(clock)*2-1)*shake,(rand(clock+2)*2-1)*shake*.4);renderBackground(cam,clock);
 ...
 }else hero(frame,p.x-game.cam,G-p.z-(p.evade>0&&p.evadeKind==='back'?Math.sin(p.evade/.18*Math.PI)*7:0)+(p.moving&&p.z===0?Math.sin(p.stride/14*Math.PI)*.7:Math.sin(clock*2)*.6));
...
function events(){for(const e of game.events.splice(0)){if(e.type==='emberWarning'){tone(180,.25,.025,'triangle',1.2);}else if(e.type==='emberBurst'){noise(.3,.15,1600);for(let i=0;i<20;i++)particles.push({x:e.data+(Math.random()-.5)*100,y:G,vx:(Math.random()-.5)*60,vy:-40-Math.random()*60,life:.3+Math.random()*.4,size:2,color:'#e9ad6977'});}else if(e.type==='checkpoint'){saveCheckpoint();}else if(e.type==='house'||e.type==='chapter'){clearTouch();keys.clear();particles.length=0;ghosts.length=0;transition=.65;if(ambient)ambient.gain.setTargetAtTime(e.type==='house'?.2:.8,audio.currentTime,.45);$('story-copy').hidden=true;storyUntil=0;captionUntil=0;toastUntil=0;tone(e.type==='house'?98:110,.8,.035,'sine',.8);}else if(e.type==='inspect'){captionUntil=0;tone(e.data.id==='drawing'?220:147,.45,.016,'sine',.97);$('story-title').textContent=e.data.title;$('story-text').textContent=e.data.text;$('story-copy').hidden=false;storyUntil=clock+7;}else if(e.type==='gunshot'){noise(.16,.22,2300);tone(75,.12,.1,'triangle',.4);for(let i=0;i<9;i++)particles.push({x:e.data.x,y:G-e.data.z,vx:e.data.face*(10+Math.random()*45),vy:-8-Math.random()*18,life:.3+Math.random()*.4,size:3+Math.random()*3,color:'#a1988666'});}else if(e.type==='throw'){noise(.1,.1,3000);tone(460,.08,.04,'triangle',.65)}else if(e.type==='rise'){noise(.35,e.data.heavy?.1:.05,300);for(let i=0;i<16;i++)particles.push({x:e.data.x+(Math.random()-.5)*40,y:G,vx:(Math.random()-.5)*60,vy:-Math.random()*70,life:.5+Math.random()*.4,size:2,color:'#6d756177'})}else if(e.type==='warning'){tone(e.data.type==='spitter'?260:e.data.type==='heavy'?100:180,.13,.035,'triangle',1.3)}else if(e.type==='jump'){noise(.08,.04,1300)}else if(e.type==='perfect'){tone(520,.12,.09,'sine',1.5);$('toast').textContent='ТОЧНЫЙ РЫВОК · КОНТРУДАР ГОТОВ';toastUntil=clock+1.4}else if(e.type==='parry'){tone(780,.15,.08,'triangle',1.4)}else if(e.type==='reflectedHit'){noise(.08,.12,1800);tone(320,.12,.06,'triangle',.7)}else if(e.type==='break'){noise(.16,.18,1300);$('toast').textContent='СТОЙКА СЛОМАНА';toastUntil=clock+1.1}else if(e.type==='land'){for(let i=0;i<5;i++)particles.push({x:e.data+(Math.random()-.5)*22,y:G,vx:(Math.random()-.5)*50,vy:-Math.random()*30,life:.2+Math.random()*.2,size:2,color:'#aa967055'})}else if(e.type==='caption'){$('caption').textContent=e.data;captionUntil=clock+5.5}else if(e.type==='toast'){$('toast').textContent=matchMedia('(pointer: coarse)').matches?e.data.replace('НАЖМИ C','НАЖМИ «НАВЬ»'):e.data;toastUntil=clock+5}else if(e.type==='sparks'){for(let i=0;i<14;i++)particles.push({x:e.data.x,y:G-60,vx:(Math.random()-.5)*180,vy:-50-Math.random()*130,life:.25+Math.random()*.5,size:i%3?2:3,color:['#ead9ac','#c38863','#a56153','#867794'][i%4]})}else if(e.type==='swing'){noise(.12,.15,3400);tone(160,.13,.05,'triangle',.4)}else if(e.type==='hit'){noise(.12,.3,1000);tone(70,.14,.2,'sine',.45)}else if(e.type==='kill'){tone(180,.5,.06,'triangle',.5)}else if(e.type==='hurt'){noise(.18,.22,450);tone(90,.2,.13,'sawtooth',.4)}else if(e.type==='dodge'){noise(.2,.08,1800)}else if(e.type==='power'){$('toast').textContent='';toastUntil=0;tone(110,1.5,.15,'sawtooth',2);tone(82,2,.08)}else if(e.type==='enemyStrike'){if(e.data.heavy){noise(.18,.16,200);game.shake=Math.max(game.shake,2)}}else if(e.type==='dead'){showMenu('ПУТЬ ОБОРВАЛСЯ','Река помнит твоё имя',game.chapter===5?'Укол — присядь. Волна — прыгни. Замах сверху — рывок за спину.':game.chapter===4?'Угли сначала вспыхивают светом. Перепрыгни огонь или заманивай в него врагов.':game.chapter===3?'Под высоким копьём присядь. Низкий подсек перепрыгни. Кортик прерывает замах.':'Перед ударом враг вспыхивает. Уйди рывком или подпрыгни.');tone(55,2,.1)}else if(e.type==='won'){if(game.chapter===1){game.enterHouse();}else if(game.chapter<5){saveCheckpoint();showMenu(`ЭПИЗОД ${game.chapter} / 5 ПРОЙДЕН`,game.level.name,game.level.outro);$('continue-level').hidden=false;$('restart').hidden=true;tone(164.81,1,.04);}else{const m=Math.floor(game.time/60),sec=Math.floor(game.time%60).toString().padStart(2,'0');showMenu('ПЯТЬ ДОРОГ ПРОЙДЕНО','Перед дверью Палия',`${game.totalKills+game.kills} теней · ${m}:${sec} · Точных уклонений: ${game.stats.perfectDodges}. Семья не найдена. Внешний двор взят. За дубовой дверью ещё ждёт Палий.`);tone(164.81,2,.06);setTimeout(()=>tone(196,2,.06),300);setTimeout(()=>tone(220,3,.05),700);}}}}

function loop(now){if(phonePortrait()!==document.body.classList.contains('phone-portrait'))fitViewport();const dt=Math.min(.05,(now-last)/1000||0);last=now;if(game.mode!=='paused'){clock+=dt;transition=Math.max(0,transition-dt);}game.tick(dt,{left:keys.has('a'),right:keys.has('d'),attack:keys.has('j'),crouch:keys.has('x')||crouchToggle});events();if(['playing','house'].includes(game.mode)){for(let i=particles.length-1;i>=0;i--){let p=particles[i];p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=330*dt;if(p.life<=0)particles.splice(i,1)}for(let i=ghosts.length-1;i>=0;i--){ghosts[i].life-=dt*3;if(ghosts[i].life<=0)ghosts.splice(i,1)}if(game.player.evade>0)ghosts.push({x:game.player.x,z:game.player.z,life:1});if(game.player.moving&&game.player.z===0){footstep+=dt;if(footstep>(game.scene==='house'?.72:.32)){noise(.035,.06,350);footstep=0;for(let j=0;j<2;j++)particles.push({x:game.player.x-game.player.face*9,y:G-1,vx:-game.player.face*(12+Math.random()*15),vy:-12-Math.random()*16,life:.22,size:2,color:'#9a897044'})}}}
 const inHouse=game.scene==='house';if(document.body.dataset.scene!==game.scene)document.body.dataset.scene=game.scene;const focus=inHouse&&game.mode==='house'?game.houseFocus():null;$('interact-hint').hidden=!focus||!$('intro').hidden;$('interact-hint').textContent=focus?(focus.id==='door'?'К СЕЧИ →':(matchMedia('(pointer: coarse)').matches?'':'Z · ')+focus.title):'';if(clock>storyUntil||!inHouse)$('story-copy').hidden=true;document.querySelector('[data-key="q"]').textContent=inHouse?'ОСМОТР':'ОТСКОК';document.querySelector('[data-key="j"]').textContent=inHouse?'ОСМОТР':'САБЛЯ';
