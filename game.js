...
let last=0,clock=0,captionUntil=0,toastUntil=0,soundOn=true,audio=null,soundscape=null,audioSleep=null,footstep=0,ready=false;
...
function start(fresh=false){if(!ready||phonePortrait())return;clearTouch();if(fresh){savedCheckpoint=null;try{localStorage.removeItem(CHECKPOINT_KEY)}catch{}}
 ...
 particles.length=0;ghosts.length=0;keys.clear();$('intro').hidden=true;$('menu').hidden=true;$('story-copy').hidden=true;$('hud').hidden=false;storyUntil=0;transition=.45;canvas.focus();footstep=Math.floor(game.player.stride/56);if(soundOn)initAudio();syncAudio();soundLabel();}
