...
function showMenu(label,title,copy,resume=false){$('endlabel').textContent=label;$('endtitle').textContent=title;$('endcopy').textContent=copy;$('resume').hidden=!resume;$('restart').textContent=savedCheckpoint?'С КОНТРОЛЬНОЙ ТОЧКИ':resume?'НАЧАТЬ ЗАНОВО':'ПРОЙТИ ЕЩЁ РАЗ';$('new-run').hidden=!savedCheckpoint;$('menu').hidden=false}
...
function walkingHero(x,y,alpha=1){
 ...
 for(const [img,weight] of [[art.walk,1-blend],[art.demonWalk,blend]]){if(!img||weight<=0)continue;const fw=img.width/4,fh=img.height/2;ctx.globalAlpha=base*weight;ctx.drawImage(img,index%4*fw,Math.floor(index/4)*fh,fw,fh,-66,-129,133,133);}
...
function renderBackground(cam,t){
 if(game.scene==='house'){renderHouse(cam,t);return;}if(game.scene==='sich'){renderSich(cam,t);return;}
...
function fitViewport(){
 ...
 if(width!==W){W=width;canvas.width=W;}game.viewWidth=W;
}
